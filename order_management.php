...
if (isset($_GET['func'])) {
    ...
    $func='list_order_management';
}
if ($func=='list_order_management'){
...
?>
    ...
    <div class = "container" style="width: 80%;">
        <div class="container" style="width: 60%;  animation: fadeInAnimation ease 2s; background-color: white; padding-left:0px;margin-left:auto;margin-right:auto;margin-top:0px;">
            <div class='container' style='width:100%; margin-left:0px'>
                <div class = 'hb25' style='width: 70%; background-color: white; margin-left:auto;margin-right:auto; text-align:left; padding:10px; '>Orders Management</div>
                <div class = 'h15' style='width: 26%; background-color: white; margin-left:auto;margin-right:0; margin-top:auto; margin-bottom:auto; '>
                    <a class='icon' href='order_management.php?func=list_order_management&view=notcompleted'><button class='button' style='font-size:12px; color: black; background:orange; width:100px; '>Not completed</button></a>
                    <a class='icon' href='order_management.php?func=list_order_management&view=completed'><button class='button' style='font-size:12px; color: black; background:orange; width:100px; '>Completed</button></a>
                </div>
            ...
            $sql="  select order_no, DATE_FORMAT(order_date,' %M %D, %Y') as order_date, subtotal, tax, total, card_no, order_ship_name
                    ...
                    from orders 
                    where order_status $view_status  order by order_no";
            ...
            while($row = mysqli_fetch_array($order, MYSQLI_BOTH)) {
                ...
                $sql="  select o.order_no, p.product_no, p.product_name, p.product_artist, p.product_size_height, round(p.product_size_height/2.54,1) as size_height_inches
                        , p.product_size_width, round(p.product_size_width/2.54,1) as size_width_inches, p.product_material, p.product_price,o.order_status
                        ...
                        where o.order_no = d.order_no and p.product_no = d.product_no and o.order_status$view_status 
                            and o.order_no = '".$order_no."'  order by d.delivery_date";
                ...
                while($row = mysqli_fetch_array($result, MYSQLI_BOTH)) {    
                    ...
                    if ($group_date_delivery == $row['delivery_date']){
                        $show_date = '';
                        $new_group=0;
                    }else{
                        $show_date = 'Delivery: '.$row['delivery_day'];
                        $group_date_delivery= $row['delivery_date'];
                        $new_group = 1;
                    }
                ?>
                    <div class='container_vertical' style='margin-left:0px;text-align:left;margin-top:10px'>
                        <div class = 'hb15' style="font-weight:bold;text-align:left;margin-top:10px;padding-left:10px;"><?php echo $show_date; ?></div>
                        <?php if ($new_group ==1){ 
                                echo "<div class = 'hb15' style='font-weight:bold;margin-left:10px;text-align:left;margin-top:10px;'>Status: $order_status</div>";
                                if ($order_status =='In process'){
                                    echo "<a class='icon' href='order_management_action.php?action=shiporder&order_no=$order_no&group_date=$group_date_delivery'><button class='button' style='font-size:12px; color: black; background:bisque; width:120px; '> Ship</button></a>";
                                    
                                }else if($order_status =='Shipped'){
                                    echo "<a class='icon' href='order_management_action.php?action=deliverorder&order_no=$order_no&group_date=$group_date_delivery'><button class='button' style='font-size:12px; color: black; background:bisque; width:120px; margin:10px;'> Deliver</button></a>";
                    ...
                    <div class="container_horizon" style="width: 100%; padding-left:20px;margin-left:auto;margin-right:auto;margin-top:10px;padding-top:0px;">
                        <div class="container" style="width: 15%;margin-left:0px;margin-right:auto;margin-top:0px;padding-top:0px;padding-left:0px;">
                        ...
                        <div class="container_vertical" style = 'width:20%; height:auto;margin-right:auto; margin-top:0px;padding-top:0px;'>
                            <?php
                                if ($product_status =='In process'){
                                    echo "<a class='icon' href='order_management_action.php?action=cancelitem&order_no=$order_no&product_no=$product_no'><button class='button' style='font-size:12px; color: black; background:bisque; width:120px; '>Cancel item</a>";
                                    ...
                                    echo "<div class='hb15' style='padding:20px'>Delivered</div>";
